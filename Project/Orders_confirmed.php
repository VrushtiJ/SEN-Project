...
<body>
	...
	<div class="navbar">
		<div class="navbar-inner">
			<div class="container-fluid">
				...
				<ul class="dropdown-menu notifications" style="overflow: scroll; width: 450px; height: 550;">
				<?php
		...
		<div class="row-fluid">
			...
			<ul class="breadcrumb">
				<li>
					...
					<a href="#">Orders</a>
					<i class="icon-angle-right"></i>
					<a href="#">Orders to be confirmed</a>
				</li><br> <br>
			...
			<a href="#" class="btn btn-primary">Save changes</a>
				...
				<div class="box span12">
					<div class="box-header" data-original-title>
						<h2><i class="halflings-icon white user"></i><b><font size="3">New Orders</font></h2>
					
					</div>
					<br>
	<img id="bottom" src="bottom.png" alt="">
				<div class="box-content" style=" height: auto; font-size: 14px; overflow: auto;">
	...
	mysqli_select_db($con,"order_manager")or die(mysqli_connect_error($con));
     ...
     $sql ="SELECT orders.order_id,orders.Retailerid,orders.order_date,orders.bill_amt,orders.parent_order_id,retailers.r_name,retailers.group_id FROM orders,retailers WHERE confirmation_status='0' AND retailers.Retailerid=orders.Retailerid";
	...
	$i=0;
	     ...
	     {
	          $g_id=$row['group_id'];
	          $grp="SELECT * from groups where group_id='$g_id'";
	          $g_res=mysqli_query($con,$grp);
	          if($g_row=mysqli_fetch_assoc($g_res))
     	          $g_name=$g_row['groupname'];
			$name = $row['r_name'];
			$r_id=$row['Retailerid'];
			$o_id=$row['order_id'];
			$date = $row['order_date'];
			$defective=$row['parent_order_id'];
			$amt = $row['bill_amt'];
			$i++;
			$enddate = date("Y-m-d"); 
			$d=strtotime("-1 Months");
			$strtdate =  date("Y-m-d h:i:sa", $d);
			$sql = "SELECT * FROM orders WHERE Retailerid ='$r_id' AND order_date >= '$strtdate' AND order_date <='$enddate'";
			$res= mysqli_query($con, $sql);
			if($res) ///count to be checked
			{				
				$count =0;
				$bill_amount = 0;
				$pcount = 0;
				...
				$factor = 50000;
					...
					{
				     	$count++;
			     		$bill_amount = $bill_amount + $rows["bill_amt"];
					     if($rows["status"])
					     {
					     	$payment_done = $payment_done + $rows["bill_amt"];
					     }
					}	
				if($bill_amount!=0){
					$to_add = $payment_done/$bill_amount;
			     }
				else{ 
					$to_add =0;
				}
				 
				$rating = .4*$count+.4*$bill_amount/$factor + .2*$to_add; 
     ...
     ?>
	  <tbody>
		...
		<td class="center"><?php echo $defective; ?></td>
		<td>
		<li class="buttons">
     	    <input type="hidden" name="form_id" value="992796" />
		    <ul class="no_bullets" id="button34">
		    
				<a href="view_details_status.php?value=<?php echo $o_id; ?>&rname=<?php echo $name;?>&id=<?php echo $r_id; ?>">    
				<li value="<?php echo $o_id; ?>">  
				     <input id="saveForm" class="btn btn-primary13" align="middle" type="submit" name="accept" value="View Details" /></a>
			</ul>
	     </li>
		</td>
		<td>
		<li class="buttons">
		    ...
		    <ul class="no_bullets" id="button34">
				...
				<a href="confirm_status.php?value=<?php echo $o_id; ?>">   
				<li value="<?php echo $o_id; ?>">  
				     <input id="saveForm" class="btn btn-primary11" align="middle" type="submit" name="accept" value="Accept" /> 		                   </a><a>
		               <input id="saveForm1" class="btn btn-primary12" align="middle" type="submit" name="submit" value="Reject"/></a>
	               </li>
			</ul>
	     </li>
		</td>
